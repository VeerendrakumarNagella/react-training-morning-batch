...
for (let i = 0; i < usersData.length; i++) {
  ...
  modifiedUsersData.push(userObj);
}

const mapUsersData = usersData.map((user) => {
...
duplicateArr.forEach((val) => (count = count + val));

for (let i = 0; i < duplicateArr.length; i++) {
  count1 += duplicateArr[i]; // count1 = count1 + duplicateArr[i];
}
